...
namespace photosynth
{
    ...
    juce::var PhotoSynthAudioProcessor::createBridgeState() const
    {
        auto patchToVar = [](const PatchParameters& p)
        {
            juce::DynamicObject::Ptr root = new juce::DynamicObject();
            root->setProperty("cutoffOffset", p.cutoffOffset);
            root->setProperty("resonance", p.resonance);
            root->setProperty("lfoRate", p.lfoRate);
            root->setProperty("lfoDepth", p.lfoDepth);
            root->setProperty("masterVolume", p.masterVolume);
            root->setProperty("attackTime", p.attackTime);
            root->setProperty("decayTime", p.decayTime);
            root->setProperty("sustainLevel", p.sustainLevel);
            root->setProperty("releaseTime", p.releaseTime);
            root->setProperty("threshold", p.threshold);
            root->setProperty("detuneCents", p.detuneCents);
            root->setProperty("unisonVoices", p.unisonVoices);
            root->setProperty("fmRatio", p.fmRatio);
            root->setProperty("fmAmount", p.fmAmount);
            root->setProperty("subOscLevel", p.subOscLevel);
            root->setProperty("reverbMix", p.reverbMix);
            root->setProperty("delayMix", p.delayMix);
            root->setProperty("temporalEraVal", p.temporalEraVal);
            root->setProperty("opticalFocusDepth", p.opticalFocusDepth);
            root->setProperty("gridSymmetryDensity", p.gridSymmetryDensity);
            root->setProperty("lightAzimuthAngle", p.lightAzimuthAngle);
            root->setProperty("lightElevationAngle", p.lightElevationAngle);
            root->setProperty("chromaticClash", p.chromaticClash);
            root->setProperty("semanticDensityWeight", p.semanticDensityWeight);
            root->setProperty("bodyDamping", p.physicalModel.bodyDamping);
            root->setProperty("acousticWeight", p.physicalModel.acousticWeight);
            root->setProperty("tapeFlutterSpeed", p.physicalModel.tapeFlutterSpeed);
            root->setProperty("tapeFlutterDepth", p.physicalModel.tapeFlutterDepth);
            root->setProperty("analogSaturationWarmth", p.physicalModel.analogSaturationWarmth);
            root->setProperty("engineType", (int) p.engineType);
            return juce::var(root.get());
        };

        auto metricsToVar = [](const ImageMetrics& m)
        {
            juce::DynamicObject::Ptr o = new juce::DynamicObject();
            o->setProperty("brightness", m.brightness);
            o->setProperty("saturation", m.saturation);
            o->setProperty("complexity", m.complexity);
            o->setProperty("timbreDna", m.timbreDna);
            o->setProperty("seedNumber", (int) m.seedNumber);
            return juce::var(o.get());
        };

        juce::DynamicObject::Ptr state = new juce::DynamicObject();
        state->setProperty("patch", patchToVar(currentPatch));
        state->setProperty("metrics", metricsToVar(currentMetrics));
        state->setProperty("morphAmount", apvts.getRawParameterValue("morphAmount")->load());
        state->setProperty("bpm", apvts.getRawParameterValue("bpm")->load());
        state->setProperty("hasImageA", hasLoadedImageA);
        state->setProperty("hasImageB", hasLoadedImageB);
        return juce::var(state.get());
    }

    bool PhotoSynthAudioProcessor::setParameterFromBridge(const juce::String& parameterID, double value)
    {
        if (auto* param = apvts.getParameter(parameterID))
        {
            param->setValueNotifyingHost(param->convertTo0to1((float) value));
            if (parameterID == "morphAmount")
                applyMorphFromLoadedImages((float) value);
            return true;
        }
        return false;
    }

    void PhotoSynthAudioProcessor::playNoteFromBridge(int note, float velocity)
    {
        keyboardState.noteOn(1, note, (juce::uint8) juce::jlimit(1, 127, (int) std::round(velocity * 127.0f)));
    }

    void PhotoSynthAudioProcessor::releaseNoteFromBridge(int note)
    {
        keyboardState.noteOff(1, note, (juce::uint8) 0);
    }

    void PhotoSynthAudioProcessor::panicFromBridge()
    {
