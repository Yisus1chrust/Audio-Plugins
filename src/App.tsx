...
export default function App() {
  ...
  // JUCE bridge sync: web UI -> C++ processor and C++ processor -> web UI
  useEffect(() => {
    void pushPatchToHost(patch);
  }, [patch]);

  useEffect(() => {
    pushTransportToHost(morphValue, bpm);
  }, [morphValue, bpm]);

  useEffect(() => {
    return subscribeBridgeState((state) => {
      if (typeof state.bpm === 'number') {
        setBpm(Math.max(40, Math.min(220, Math.round(state.bpm))));
      }

      if (typeof state.morphAmount === 'number') {
        setMorphValue(Math.max(0, Math.min(1, state.morphAmount)));
      }

      if (state.patch) {
        setPatch((prev) => {
          if (!prev) return prev;
          const merged = { ...prev, ...state.patch } as PatchParameters;
          globalAudioEngine.updateParameters(merged);
          return merged;
        });
      }
    });
